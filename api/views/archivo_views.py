...
def procesar_archivo_async(archivo_carga_id: int, tipo: str):
    ...
    try:
        ...
        logger.info(f"Iniciando procesamiento de archivo {archivo_carga_id} tipo {tipo}")
        ...
        logger.info(f"Procesamiento completado para archivo {archivo_carga_id}: {resultado}")
        
    except ArchivoCarga.DoesNotExist:
        logger.error(f"Archivo con ID {archivo_carga_id} no encontrado")
    except Exception as e:
        logger.error(f"Error crítico procesando archivo {archivo_carga_id}: {str(e)}", exc_info=True)
        # Asegurar que el estado se actualice incluso en errores críticos
        if archivo_carga:
            try:
                archivo_carga.estado = 'ERROR'
                archivo_carga.agregar_error(0, f"Error crítico durante procesamiento: {str(e)}")
                archivo_carga.save(update_fields=['estado'])
            except Exception as save_error:
                logger.error(f"Error adicional al guardar estado de error: {save_error}")
...
def estado_procesamiento(request, archivo_id):
    ...
    serializer = EstadoProcesamientoSerializer({
        ...
        'filas_exitosas': archivo_carga.filas_procesadas - archivo_carga.filas_errores,

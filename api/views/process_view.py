...
class ProcesarArchivoView(APIView):
    ...
    def post(self, request, archivo_id):
        ...
        if archivo.procesado:
            return Response({"error": "El archivo ya fue procesado."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            if archivo.tipo == "pacientes":
                resultado = pacientes_loader.cargar_pacientes(archivo.archivo.path)
            else:
                return Response({"error": f"Tipo de archivo '{archivo.tipo}' no reconocido."},
                                status=status.HTTP_400_BAD_REQUEST)

            archivo.procesado = True
            archivo.save()

            return Response({"mensaje": "Archivo procesado correctamente.", "resultado": resultado})

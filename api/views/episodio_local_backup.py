...
class EpisodioViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, request):
        ...
        episodios_activos = (
            self.get_queryset().filter(fecha_egreso__isnull=True).count()
        )
        ...
        por_especialidad = {}
        especialidades = (
            self.get_queryset().values_list("especialidad", flat=True).distinct()
        )
    ...
    def activos(self, request):
        ...
        for backend in filter_backends:
            episodios_activos = backend().filter_queryset(
                request, episodios_activos, self
            )

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if self.verbosity >= 2:
                ...
                self.stdout.write(f"📊 Archivos encontrados: {list(excel_files.keys())}")
            ...
            if options['export_combined']:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✅ Datos procesados exportados a: {export_path}")
                )
        ...
        for filename in required_files:
            ...
            if not os.path.exists(file_path):
                ...
                if os.path.exists(alt_path):
                    ...
                    raise CommandError(f"Archivo no encontrado: {filename} en {folder_path}")
    ...
    def _list_excel_files(self, folder_path):
        ...
        required_files = ['excel1.xlsx', 'excel2.xlsx', 'excel3.xlsx', 'excel1.xls', 'excel2.xls', 'excel3.xls']
    ...
    def _simulate_import(self, mapped_data):
        ...
        if pacientes and self.verbosity >= 2:
            ...
            self.stdout.write(f"    📋 Ejemplo: RUT {sample_patient.get('rut', 'N/A')}, "
        ...
        if episodios and self.verbosity >= 2:
            ...
            self.stdout.write(f"    📋 Ejemplo: CMBD {sample_episode.get('episodio_cmbd', 'N/A')}")
        
        self.stdout.write(f"  💼 Gestiones: {len(gestiones)} registros")
        if gestiones and self.verbosity >= 2:
            sample_gestion = gestiones[0]
            self.stdout.write(f"    📋 Ejemplo: Fecha {sample_gestion.get('fecha_ingreso', 'N/A')}")
    ...
    def _show_import_results(self, results):
        ...
        self.stdout.write(f"  👥 Pacientes: {p_created} creados, {p_updated} actualizados")
        if p_errors > 0:
        ...
        self.stdout.write(f"  📝 Episodios: {e_created} creados, {e_updated} actualizados")
        if e_errors > 0:
        ...
        self.stdout.write(f"  💼 Gestiones: {g_created} creadas, {g_updated} actualizadas")
        if g_errors > 0:

...
from api.models import Paciente, Episodio, Gestion, Cama, Transferencia
...
class DatabaseImporter:
        ...
        for paciente_data in pacientes_data:
            try:
                ...
                if not rut:
                    ...
                    self.error_details.append(f"Paciente sin RUT en episodio {paciente_data.get('episodio_cmbd')}")
                    continue
                ...
                paciente, created = Paciente.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'prevision_1': str(paciente_data.get('prevision_1', 'OTRO'))[:20],
                        'prevision_2': str(paciente_data.get('prevision_2', ''))[:20] if paciente_data.get('prevision_2') else None,
                ...
                if created:
                    ...
                    if not paciente.fecha_nacimiento and paciente_data.get('fecha_nacimiento'):
                        paciente.fecha_nacimiento = paciente_data.get('fecha_nacimiento')
                        updated = True
                    
                    # Actualizar previsión si no es 'OTRO' o si tenemos mejor información
                    new_prevision_1 = paciente_data.get('prevision_1', '')[:20]  # Truncar a 20 chars
                    if new_prevision_1 and (paciente.prevision_1 == 'OTRO' or not paciente.prevision_1):
                    ...
                    if new_prevision_2:
                        new_prevision_2 = str(new_prevision_2)[:20]  # Truncar a 20 chars
                    ...
                    logger.debug(f"Comparando sexo para {rut}: actual='{paciente.sexo}', nuevo='{new_sexo}'")
                    if new_sexo != 'O' and paciente.sexo == 'O':
                        logger.info(f"Actualizando sexo de {rut}: '{paciente.sexo}' -> '{new_sexo}'")
        ...
        for cama_data in camas_data:
            try:
                ...
                if created:
                    ...
                    if cama_data.get('habitacion') and cama_data['habitacion'] != cama.habitacion:
    ...
    def _import_episodios(self, episodios_data: List[Dict]) -> None:
        ...
        for episodio_data in episodios_data:
            try:
                ...
                paciente = self._find_paciente_for_episodio(episodio_data, episodio_cmbd)
                if not paciente:
                    self.results['episodios']['errors'] += 1
                    self.error_details.append(f"No se encontró paciente para episodio {episodio_cmbd}")
                    continue
                ...
                episodio, created = Episodio.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'tipo_actividad': episodio_data.get('tipo_actividad', 'Hospitalización'),
                        'especialidad': episodio_data.get('especialidad'),
                        'inlier_outlier_flag': episodio_data.get('inlier_outlier_flag'),
                        'estancia_prequirurgica': episodio_data.get('estancia_prequirurgica'),
                        'estancia_postquirurgica': episodio_data.get('estancia_postquirurgica'),
        ...
        logger.info(f"Importación de episodios completada: {self.results['episodios']['created']} creados, {self.results['episodios']['updated']} actualizados, {self.results['episodios']['errors']} errores")
        ...
        for gestion_data in gestiones_data:
            try:
                ...
                try:
                    ...
                    self.error_details.append(f"No se encontró episodio {episodio_cmbd} para gestión")
                    continue
                ...
                gestion_data_clean = {k: v for k, v in gestion_data_clean.items() if v is not None}
                ...
                self.results['gestiones']['errors'] += 1
                error_msg = f"Error validación gestión para episodio {episodio_cmbd}: {str(e)}"
                self.error_details.append(error_msg)
                logger.error(error_msg)
    ...
    def _find_paciente_for_episodio(self, episodio_data: Dict, episodio_cmbd: int) -> Optional[Paciente]:
        """
        ...
        if nombre_paciente:
            try:
                return Paciente.objects.filter(nombre__icontains=nombre_paciente).first()
            except:
                pass
    ...
    def _find_cama(self, episodio_data: Dict) -> Optional[Cama]:
        ...
        try:
            ...
            logger.warning(f"No se encontró cama {cama_codigo} en habitación {habitacion}")
            return None
        except Cama.MultipleObjectsReturned:
            # Si hay múltiples camas con el mismo código, tomar la primera
            logger.warning(f"Múltiples camas con código {cama_codigo}, tomando la primera")
        ...
        for transferencia_data in transferencias_data:
            try:
                ...
                try:
                    ...
                    self.results['transferencias']['errors'] += 1
                    self.error_details.append(f"No se encontró episodio {episodio_cmbd} para transferencia")
                    continue
                ...
                if gestion_created:
                    logger.debug(f"Gestión de transferencia creada para episodio {episodio_cmbd}")
                ...
                transferencia, created = Transferencia.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'motivo_cancelacion': transferencia_data.get('motivo_cancelacion'),
                        'motivo_rechazo': transferencia_data.get('motivo_rechazo'),
                        'tipo_traslado': transferencia_data.get('tipo_traslado', ''),
                        'motivo_traslado': transferencia_data.get('motivo_traslado', ''),
                        'centro_destinatario': transferencia_data.get('centro_destinatario', '')
                    }
                )
                ...
                if created:
                    ...
                    # Actualizar datos si es necesario
                    updated = False
                    if transferencia_data.get('estado') and transferencia.estado != transferencia_data.get('estado'):
                        transferencia.estado = transferencia_data.get('estado')
                        updated = True
                    if transferencia_data.get('motivo_cancelacion') and transferencia.motivo_cancelacion != transferencia_data.get('motivo_cancelacion'):
                        transferencia.motivo_cancelacion = transferencia_data.get('motivo_cancelacion')
                        updated = True
                    if transferencia_data.get('motivo_rechazo') and transferencia.motivo_rechazo != transferencia_data.get('motivo_rechazo'):
                        transferencia.motivo_rechazo = transferencia_data.get('motivo_rechazo')
                        updated = True
                    if transferencia_data.get('tipo_traslado') and transferencia.tipo_traslado != transferencia_data.get('tipo_traslado'):
                        transferencia.tipo_traslado = transferencia_data.get('tipo_traslado')
                        updated = True
                    if transferencia_data.get('motivo_traslado') and transferencia.motivo_traslado != transferencia_data.get('motivo_traslado'):
                        transferencia.motivo_traslado = transferencia_data.get('motivo_traslado')
                        updated = True
                    if transferencia_data.get('centro_destinatario') and transferencia.centro_destinatario != transferencia_data.get('centro_destinatario'):
                        transferencia.centro_destinatario = transferencia_data.get('centro_destinatario')
                        updated = True
                    
                    if updated:
                        transferencia.save()
                        self.results['transferencias']['updated'] += 1
                        logger.debug(f"Transferencia actualizada para episodio {episodio_cmbd}")
    ...
    def _get_results_summary(self) -> Dict:
        ...
        """
        total_processed = sum(
            self.results[model]['created'] + self.results[model]['updated'] + self.results[model]['errors']
        ...
        return {
            ...
            'errors': self.error_details[:50]  # Limitar a 50 errores para no saturar logs
        }

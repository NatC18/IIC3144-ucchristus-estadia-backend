...
class ExcelProcessor:
    ...
    def _load_single_excel(self, file_path: Path, file_name: str) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if episodio_col:
                logger.info(f"Encontrada columna de episodio '{episodio_col}' en {file_name}")
                ...
                possible_cols = [col for col in df.columns if 'episodio' in col.lower()]
                if possible_cols:
                    logger.info(f"Usando columna {possible_cols[0]} como episodio_cmbd en {file_name}")
                    df['episodio_cmbd'] = df[possible_cols[0]]
                else:
                    logger.error(f"{file_name} no tiene columna de episodio identificable")
                    return None
            ...
            if initial_count != final_count:
                logger.warning(f"{file_name}: Eliminadas {initial_count - final_count} filas sin episodio_cmbd")
            
            # Convertir episodio_cmbd a string para asegurar consistencia
            df['episodio_cmbd'] = df['episodio_cmbd'].astype(str)
            
            logger.info(f"{file_name} cargado: {len(df)} filas, {len(df.columns)} columnas")
            return df
    ...
    def combine_data(self) -> bool:
        ...
        """
        try:
            if not all([self.excel1_df is not None, self.excel2_df is not None, self.excel3_df is not None]):
            ...
            if not all('episodio_cmbd' in df.columns for df in [excel1_clean, excel2_clean, excel3_clean]):
                logger.error("No se pudo encontrar columna de episodio en todos los archivos")
                return False
            ...
            possible_excel1_cols = {
                'Tipo Actividad': ['Tipo Actividad', 'tipo_actividad', 'Tipo de Actividad'],
                'Estancia Inlier / Outlier': ['Estancia Inlier / Outlier', 'estancia_inlier_outlier', 'Inlier/Outlier'],
                'Especialidad médica de la intervención (des)': ['Especialidad médica de la intervención (des)', 'especialidad_medica', 'Especialidad'],
                'Estancias Prequirurgicas Int  -Episodio-': ['Estancias Prequirurgicas Int  -Episodio-', 'estancia_prequirurgica', 'Prequirurgica'],
                'Estancias Postquirurgicas Int  -Episodio-': ['Estancias Postquirurgicas Int  -Episodio-', 'estancia_postquirurgica', 'Postquirurgica'],
                'Estancia Norma GRD': ['Estancia Norma GRD', 'estancia_norma_grd', 'Norma GRD']
            }
            ...
            combined = pd.merge(combined, excel1_mapped, on='episodio_cmbd', how='left', suffixes=('', '_excel1'))
            ...
            combined = pd.merge(combined, excel3_mapped, on='episodio_cmbd', how='left', suffixes=('', '_excel3'))
            
            self.combined_df = combined
            
            # Estadísticas de combinación
            total_episodios = len(combined)
            episodios_con_excel1 = len(combined.dropna(subset=[col for col in combined.columns if col.endswith('_excel1')]))
            episodios_con_excel3 = len(combined.dropna(subset=[col for col in combined.columns if col.endswith('_excel3')]))
    ...
    def _normalize_episodio_column(self, df: pd.DataFrame, source: str) -> pd.DataFrame:
        ...
        episodio_variants = [
            'Episodio CMBD', 'episodio_cmbd', 'Episodio:', 'episodio:', 
            'EPISODIO', 'episodio', 'Episodio', 'cmbd', 'CMBD'
        ]
    ...
    def _rename_columns_with_suffix(self, df: pd.DataFrame, suffix: str) -> pd.DataFrame:
        """
    ...
    def get_data_summary(self) -> Dict:
        ...
        for name, df in [('excel1', self.excel1_df), ('excel2', self.excel2_df), ('excel3', self.excel3_df)]:
            if df is not None:
    ...
    def _prepare_pacientes_data(self) -> pd.DataFrame:
        ...
        pacientes_df = pacientes_df.drop_duplicates(subset=['episodio_cmbd'], keep='first')
    ...
    def _extract_rut(self, row) -> str:
        ...
        rut_columns = [
            'RUT', 'rut', 'RUT_PACIENTE', 'rut_paciente',
            'RUT_excel2', 'rut_excel2', 'RUT_PACIENTE_excel3', 'rut_paciente_excel3'
        ]
    ...
    def _extract_nombre(self, row) -> str:
        ...
        nombre_columns = [
            'Nombre', 'nombre', 'NOMBRE_PACIENTE', 'nombre_paciente',
            'Nombre_excel2', 'nombre_excel2', 'NOMBRE_PACIENTE_excel3', 'nombre_paciente_excel3'
        ]
    ...
    def _extract_sexo(self, row) -> str:
        ...
        sexo_columns = [
            'Sexo  (Desc)', 'sexo', 'Sexo', 'sexo_desc',
            'Sexo  (Desc)_excel2', 'sexo_excel2', 'Sexo_excel3'
        ]
    ...
    def _extract_edad(self, row) -> int:
        ...
        edad_columns = [
            'Edad en años', 'edad', 'Edad', 'edad_anos',
            'Edad en años_excel2', 'edad_excel2', 'Edad_excel3'
        ]
    ...
    def _extract_prevision(self, row) -> str:
        ...
        prevision_columns = [
            'Prevision (Desc)', 'prevision', 'Convenio', 'Nombre de la aseguradora',
            'Prevision (Desc)_excel2', 'prevision_excel2', 'Convenio_excel2', 'Nombre de la aseguradora_excel2'
        ]
    ...
    def _extract_fecha_nacimiento(self, row) -> str:
        ...
        fecha_nacimiento_columns = [
            'Fecha de Nacimiento', 'fecha_nacimiento', 'fecha_nac',
            'Fecha de Nacimiento_excel2', 'fecha_nacimiento_excel2'
        ]
    ...
    def _prepare_episodios_data(self) -> pd.DataFrame:
        ...
        episodios_df = episodios_df.drop_duplicates(subset=['episodio_cmbd'], keep='first')
    ...
    def _extract_fecha_ingreso(self, row) -> str:
        ...
        fecha_columns = [
            'fecha ingreso completa', 'fecha_ingreso', 'fecha admisión', 'fecha_admision', 'fecha inicio:', 
            'fecha admisión_excel2', 'fecha_admision_excel2', 'fecha inicio:_excel2',
            'fecha_admision_excel3', 'fecha_carga_excel3'
        ]
    ...
    def _extract_fecha_alta(self, row) -> str:
        ...
        fecha_columns = [
            'fecha completa', 'fecha_alta', 'fecha alta', 'fecha_egreso',
            'fecha alta_excel2', 'fecha_alta_excel2', 'fecha_egreso_excel2'
        ]
        for col in fecha_columns:
            if col in row and not pd.isna(row[col]) and str(row[col]).strip() != '':
                fecha_str = str(row[col]).strip()
    ...
    def _extract_servicio(self, row) -> str:
        ...
        servicio_columns = [
            'servicio ingreso (descripción)', 'servicio', 'servicio (especialidad)', 'especialidad',
            'servicio ingreso (descripción)_excel2', 'servicio (especialidad)_excel2'
        ]
    ...
    def _extract_cama(self, row) -> str:
        ...
        cama_columns = [
            'cama', 'cama_codigo', 'cama_excel2', 'cama_excel3'
        ]
    ...
    def _extract_diagnostico(self, row) -> str:
        ...
        diag_columns = ['Diagnóstico   Principal', 'diagnostico_principal', 'Texto libre diagnóstico admisión']
    ...
    def _extract_estancia(self, row) -> int:
        ...
        estancia_columns = ['Estancia del Episodio', 'estancia_dias', 'Días Hospitalización', 'dias_estancia']
    ...
    def _extract_tipo_gestion(self, row) -> str:
        ...
        tipo_columns = ['¿Qué gestión se solicito?', 'tipo_gestion', 'Tipo de Solicitud', 'TIPO_MOVIMIENTO']
        for col in tipo_columns:
            if col in row and not pd.isna(row[col]):
                return str(row[col]).strip()
        return ''
    ...
    def _extract_fecha_inicio_gestion(self, row) -> str:
        ...
        fecha_columns = ['Fecha Inicio:', 'fecha_inicio_gestion', 'Fecha admisión', 'FECHA_CARGA']
        for col in fecha_columns:
            if col in row and not pd.isna(row[col]):
                fecha_str = str(row[col]).strip()
                try:
    ...
    def _extract_fecha_fin_gestion(self, row) -> str:
        ...
        fecha_columns = ['Fecha de Finalización', 'fecha_fin_gestion', 'Última Modificación']
        for col in fecha_columns:
            if col in row and not pd.isna(row[col]):
                fecha_str = str(row[col]).strip()
                try:
    ...
    def _extract_observaciones_gestion(self, row) -> str:
        ...
        obs_columns = ['Informe', 'observaciones', 'Motivo de Cancelación', 'Causa Devolución/Rechazo']
    ...
    def process_local_files(self, file_paths: Dict[str, str]) -> Dict[str, pd.DataFrame]:
        """
        ...
        logger.info(f"Procesamiento completado: Excel1={len(self.excel1_df)}, Excel2={len(self.excel2_df)}, Excel3={len(self.excel3_df)} registros")
        ...
        if 'excel1' in result and 'excel2' in result:
            ...
            combined_df = pd.merge(result['excel1'], result['excel2'], on='RUT', how='outer', suffixes=('', '_excel2'))
            ...
            if 'excel3' in result:
                ...
                for col in ['CÓDIGO EPISODIO CMBD', 'episodio_cmbd', 'CMBD', 'Episodio']:
                ...
                if excel1_episodio_col and excel3_episodio_col:
                    ...
                    excel3_for_merge = result['excel3'].rename(columns={excel3_episodio_col: excel1_episodio_col})
                    print(f"[DEBUG] Excel3 columnas: {list(excel3_for_merge.columns)}")
                    
                    # Hacer el merge por episodio
                    combined_df = pd.merge(combined_df, excel3_for_merge, on=excel1_episodio_col, how='left', suffixes=('', '_excel3'))
                    print(f"[DEBUG] Combined después de Excel3: {list(combined_df.columns)}")

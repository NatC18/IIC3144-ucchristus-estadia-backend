...
class DataMapper:
    ...
    def map_all_data(self, clean_data: Dict[str, pd.DataFrame]) -> Dict[str, List[Dict]]:
        """
        ...
        try:
            ...
            if 'combined' in clean_data and not clean_data['combined'].empty:
                ...
                self.mapped_data['pacientes'] = self._map_pacientes_from_combined(combined_df)
                self.mapped_data['camas'] = self._map_camas_from_combined(combined_df)
                self.mapped_data['episodios'] = self._map_episodios_from_combined(combined_df)
                self.mapped_data['gestiones'] = self._map_gestiones_from_combined(combined_df)
                self.mapped_data['transferencias'] = self._map_transferencias_from_combined(combined_df)
    ...
    def _map_pacientes_from_combined(self, df: pd.DataFrame) -> List[Dict]:
        ...
        df_temp = df.copy()
        df_temp['rut_normalizado'] = df_temp.apply(lambda row: self._extract_rut_from_row(row), axis=1)
        ...
        for rut, group in rut_groups:
            try:
                ...
                fecha_nacimiento = self._parse_date(self._safe_get(row, 'Fecha de Nacimiento'))
                
                # Extraer previsiones y truncar a 20 caracteres
                prevision_1_raw = self._safe_get(row, 'Convenio') or self._safe_get(row, 'Nombre de la aseguradora') or 'OTRO'
                prevision_2_raw = self._safe_get(row, 'Nombre de la aseguradora') if self._safe_get(row, 'Convenio') else None
                ...
                if paciente_data['rut'] and paciente_data['nombre']:
                    ...
                    logger.debug(f"Paciente mapeado: RUT={paciente_data['rut']}, Nombre={paciente_data['nombre']}, FechaNac={paciente_data['fecha_nacimiento']}")
                else:
                    logger.warning(f"Paciente sin datos básicos: RUT={paciente_data['rut']}, Nombre={paciente_data['nombre']}")
            ...
            try:
                ...
                if codigo_cama and codigo_cama != 'nan':
                    ...
                    if cama_key not in camas_unicas:
                        ...
                        if not habitacion or habitacion.strip() == '' or habitacion == 'nan':
    ...
    def _map_episodios_from_combined(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                fecha_ingreso = self._parse_date_universal(self._safe_get(row, 'Fecha Ingreso completa'))
                ...
                if fecha_egreso_excel2 and not pd.isna(fecha_egreso_excel2) and str(fecha_egreso_excel2) != 'nan':
                ...
                episodio_data = {
                    'episodio_cmbd': self._convert_to_int(self._safe_get(row, 'CÓDIGO EPISODIO CMBD')),
                    ...
                    'inlier_outlier_flag': self._safe_get(row, 'Estancia Inlier / Outlier'),
                    'especialidad': self._safe_get(row, 'Especialidad médica de la intervención (des)'),
                    'estancia_prequirurgica': self._safe_get_float(row, 'Estancias Prequirurgicas Int  -Episodio-'),
                    'estancia_postquirurgica': self._safe_get_float(row, 'Estancias Postquirurgicas Int  -Episodio-'),
                    'estancia_norma_grd': self._safe_get_float(row, 'Estancia Norma GRD'),
                ...
                if episodio_data['episodio_cmbd'] and episodio_data['rut_paciente']:
                    ...
                    logger.debug(f"Episodio mapeado: CMBD={episodio_data['episodio_cmbd']}, RUT={episodio_data['rut_paciente']}, Cama={episodio_data['codigo_cama']}")
                else:
                    logger.warning(f"Episodio sin datos básicos: CMBD={episodio_data['episodio_cmbd']}, RUT={episodio_data['rut_paciente']}")
    ...
    def _map_gestiones_from_combined(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                fecha_inicio = (
                    self._parse_date_universal(self._safe_get(row, 'Fecha admisión'))
                )
                ...
                gestion_data = {
                    ...
                    'informe': self._safe_get(row, 'Informe') or f'Gestión de tipo {tipo_gestion_raw}'
                }
                
                if gestion_data['episodio_cmbd']:
                    gestiones.append(gestion_data)
                    logger.debug(f"Gestión mapeada: CMBD={gestion_data['episodio_cmbd']}, Tipo={gestion_data['tipo_gestion']}")
        ...
        for idx, row in transferencia_df.iterrows():
            try:
                transferencia_data = {
                    ...
                    'centro_destinatario': self._safe_get(row, 'Centro de Destinatario'),
                    'fecha_solicitud': self._parse_date_universal(self._safe_get(row, 'Fecha admisión')),
    ...
    def _extract_nombre_from_row(self, row: pd.Series) -> str:
        ...
        nombre_cols = ['Nombre', 'nombre', 'NOMBRE_PACIENTE', 'nombre_paciente', 'Nombre del Paciente']
    ...
    def _map_episodios(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                fecha_ingreso = self._parse_date_universal(self._safe_get(row, 'fecha_ingreso'))
                
                # Debug para los primeros registros
                if idx < 3:
                    logger.info(f"Registro {idx}: episodio={episodio_cmbd}, fecha_ingreso={fecha_ingreso}")
                
                episodio_data = {
                    'episodio_cmbd': self._convert_to_int(episodio_cmbd),  # Convertir a entero
                    # Agregar información del paciente para la búsqueda
                    'rut_paciente': self._safe_get(row, 'rut'),  # Extraer RUT del paciente de los datos combinados
                    'nombre_paciente': self._safe_get(row, 'nombre'),  # Extraer nombre del paciente
                    'fecha_ingreso': fecha_ingreso,
                    'fecha_egreso': self._parse_date_universal(self._safe_get(row, 'fecha_alta')),  # Cambiado de 'fecha_egreso' a 'fecha_alta'
                    'tipo_actividad': self._safe_get(row, 'tipo_actividad', 'Hospitalización'),
                    'especialidad': self._safe_get(row, 'servicio'),  # Cambiado de 'especialidad' a 'servicio'
                    'cama': self._safe_get(row, 'cama'),  # Cambiado de 'cama_codigo' a 'cama'
                    'habitacion': self._safe_get(row, 'habitacion'),
                    'diagnostico_principal': self._safe_get(row, 'diagnostico_principal'),
                    ...
                    'estancia_prequirurgica': self._safe_get_float(row, 'estancia_prequirurgica'),
                    'estancia_postquirurgica': self._safe_get_float(row, 'estancia_postquirurgica'),
                    'estancia_norma_grd': self._safe_get_float(row, 'estancia_norma_grd')
                }
                ...
                if not episodio_data['fecha_ingreso']:
                    logger.warning(f"Episodio {episodio_data['episodio_cmbd']} sin fecha de ingreso")
    ...
    def _map_gestiones(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for idx, row in df.iterrows():
            try:
                # Usar los nombres de columna que vienen del ExcelProcessor
                episodio_cmbd = self._safe_get(row, 'episodio_cmbd')
                tipo_gestion = self._map_tipo_gestion(self._safe_get(row, 'tipo_gestion'))
                
                # Debug para los primeros registros
                if idx < 3:
                    logger.info(f"Registro {idx}: episodio={episodio_cmbd}, tipo={tipo_gestion}")
                
                gestion_data = {
                    'episodio_cmbd': self._convert_to_int(episodio_cmbd),  # Convertir a entero
                    'tipo_gestion': tipo_gestion,
                    'estado_gestion': self._map_estado_gestion(self._safe_get(row, 'estado_gestion')),
                    'fecha_inicio': self._parse_date_universal(self._safe_get(row, 'fecha_inicio')),  # Cambiado de 'fecha_inicio_gestion'
                    'fecha_fin': self._parse_date_universal(self._safe_get(row, 'fecha_fin')),  # Cambiado de 'fecha_fin_gestion'
                    'informe': self._safe_get(row, 'observaciones'),  # Cambiado de 'informe_gestion' a 'observaciones'
                    'usuario_email': self._safe_get(row, 'usuario_responsable'),  # Cambiado de 'usuario_gestion'
                ...
                if not gestion_data['tipo_gestion']:
                    logger.warning(f"Gestión sin tipo para episodio {gestion_data['episodio_cmbd']}")
                    continue
    ...
    def _convert_to_int(self, value: Any, default: Optional[int] = None) -> Optional[int]:
    ...
    def map_processed_data(self, processed_data: Dict[str, pd.DataFrame]) -> Dict[str, List[Dict]]:
        """

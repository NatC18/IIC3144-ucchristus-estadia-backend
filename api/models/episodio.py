...
class Episodio(models.Model):
    ...
    cama = models.ForeignKey('Cama', on_delete=models.SET_NULL, related_name='episodios', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.cama and not self.fecha_egreso:
            conflicto  = Episodio.objects.filter(
                cama=self.cama,
                fecha_egreso__isnull=True
            ).exclude(id=self.id).exists()
            if conflicto:
                raise ValidationError(f"La cama {self.cama.codigo_cama} ya está asignada a otro episodio activo.")

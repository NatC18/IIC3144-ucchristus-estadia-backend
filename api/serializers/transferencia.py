...
class TransferenciaUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if estado == Transferencia.ESTADO_CANCELADA and not attrs.get(
            "motivo_cancelacion"
        ):
            raise serializers.ValidationError(
                "El motivo de cancelación es obligatorio si el estado es 'cancelada'."
            )
        if estado == Transferencia.ESTADO_RECHAZADA and not attrs.get("motivo_rechazo"):
            raise serializers.ValidationError(
                "El motivo de rechazo es obligatorio si el estado es 'rechazada'."
            )
        return attrs
